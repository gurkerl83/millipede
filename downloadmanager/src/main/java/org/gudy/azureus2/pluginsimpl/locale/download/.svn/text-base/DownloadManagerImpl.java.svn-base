...
package org.gudy.azureus2.pluginsimpl.locale.download;
...
public class 
DownloadManagerImpl
	implements org.gudy.azureus2.plugins.download.DownloadManager, DownloadManagerInitialisationAdapter, DownloadBridge
{
	protected static DownloadManagerImpl	singleton;
//        protected static DownloadManager singleton;
	protected static AEMonitor				class_mon	= new AEMonitor( "DownloadManager:class");

//        public static void setSingleton(DownloadManager mgr) {
//            singleton = (DownloadManagerImpl) mgr;
//        }
...
//			if ( singleton == null ){
//
//				singleton = new DownloadManagerImpl( azureus_core, global_manager );
//			}
//
//			return( singleton );
//
...
//    public GlobalManager gm;
	public
	DownloadManagerImpl(AzureusCore core, GlobalManager gm)
	{
...
//                System.out.println("Core: " + azureus_core);
//                System.out.println("GlobalManager: " + global_manager);
...
//					DownloadImpl	dl				= null;
                                        TransferImpl    dl                              = null;

					try{
//						listeners_mon.enter();

//						dl = (DownloadImpl)download_map.get( dm );
                                                dl = (TransferImpl)download_map.get( dm ) ;
...
//							dl.destroy();
                                                        System.out.println("Download destroy called //");
...
//					DownloadImpl	download = (DownloadImpl)download_map.get( dm );
                                        TransferImpl    download = (TransferImpl)download_map.get( dm );
					if ( download != null ){

//						try{
//							download.isRemovable();
                                                        System.out.println("Test if download is removable //");
...
//				dl = (DownloadImpl)pending_dls.remove( dm );
                                dl = (TransferImpl)pending_dls.remove( dm );

				if ( dl == null ){
					
//					dl = new DownloadImpl(dm);
                                        dl = new TransferImpl(dm);
				}
...
//		boolean	for_seeding = torrent.isComplete();
                boolean for_seeding = true;

                System.out.println("TorrentFileToString: " + torrent_file.toString() +
                        " Hash: " + hash + " data_location: " + data_location.toString() +
	...
	getInitialState()
	{
	  	boolean	default_start_stopped = COConfigurationManager.getBooleanParameter( "Default Start Torrents Stopped" );
                System.out.println("DownloadManager default start stopped: " + default_start_stopped);
...
//		DownloadImpl dl = (DownloadImpl)download_map.get(dm);
		TransferImpl dl = (TransferImpl)download_map.get(dm);
		if ( dl == null ){
...
//	public static DownloadImpl[] getDownloadStatic(DownloadManager[] dm) {
	public TransferImpl[] getDownloadStatic(DownloadManager[] dm) {
...
//		return (DownloadImpl[])res.toArray(new DownloadImpl[res.size()]);
                return (TransferImpl[])res.toArray(new TransferImpl[res.size()]);
...
//		return DownloadManagerDefaultPaths.DEFAULT_HANDLER;
    ...
    @Override
    public void addExternalDownload(Future<Download> dl) {
        try {
            this.addDownloadManager((DownloadManager) dl.get());
        } catch (InterruptedException ex) {
            Logger.getLogger(DownloadManagerImpl.class.getName()).log(Level.SEVERE, null, ex);
...
//        System.out.println("GlobalManager: " + gm);
//        context.registerService(org.gudy.azureus2.plugins.download.DownloadManager.class.getName(), new org.gudy.azureus2.pluginsimpl.locale.download.DownloadManagerImpl(), null);
//        ServiceReference dlmgrRef = context.getServiceReference(org.gudy.azureus2.plugins.download.DownloadManager.class.getName());
////        singleton = (DownloadManager)context.getService(dlmgrRef);
//
//    }
//
//    @Override
