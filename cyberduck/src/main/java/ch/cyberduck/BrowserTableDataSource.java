...
//            TableCellAttributes.browserFontLeftAlignment());
    ...
    public int indexOf(Path p) { //NSView tableView, Path p) {
        return this.childs(controller.workdir()).indexOf(p);
    }

    public boolean contains(Path p) { //NSView tableView, Path p) {
...
//    protected void setObjectValueForItem(final Path item, final NSObject value, final String identifier) {
    protected void setObjectValueForItem(final Path item, final Object value, final String identifier) {
...
//        return IconCache.instance().iconForPath(item, 16);
    protected Object iconForPath(final Path item) {
//      return IconCache.instance().iconForPath(item, 16);
    	return null;
    }
	 ...
	 * @uml.property  name="tableViewCache"
	 * @uml.associationEnd  
	 */
    private AttributeCache<Path> tableViewCache = new AttributeCache<Path>(5
...
//        final NSObject cached = tableViewCache.get(item, identifier);
        final Object cached = tableViewCache.get(item, identifier);
        if(null == cached) {
...
//    }
    ...
    /**
	 * @return    the names (not full paths) of the files that the receiver promises to create at dropDestination.  This method is invoked when the drop has been accepted by the destination and the destination, in the case of another  Cocoa application, invokes the NSDraggingInfo method namesOfPromisedFilesDroppedAtDestination. For long operations,  you can cache dropDestination and defer the creation of the files until the finishedDraggingImage method to avoid  blocking the destination application.
	 * @uml.property  name="needsDisplay"
	 */
...
//    }
    ...
    protected void setNeedsDisplay() {
