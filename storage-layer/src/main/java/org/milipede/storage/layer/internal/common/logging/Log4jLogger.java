 ...
 *   email                : {jens.bruhn|sven.kaffille}@wiai.uni-bamberg.de *
 *                                                                         *
 *                                                                         *
 ***************************************************************************/
...
package org.milipede.storage.layer.internal.common.logging;
	...
	private static void configure() {
		if (!configured) {
			...
			try {
				...
				if (configURL == null) {
					String fP = System.getProperty(PROPERTIES_FILE_PROPERTY);
					
					// Check if we must replace reference to custom property '${myPropery}' with its value.
					int startInd, endInd;
					if (fP != null && (startInd = fP.indexOf("${")) >= 0 && (endInd = fP.indexOf("}")) >= 0) {
						String propertyName = fP.substring(startInd + 2, endInd);
						String propertyValue = System.getProperty(propertyName);
						fP = fP.replace("${" + propertyName + "}", propertyValue);
					}
